...
class AnalyticsAgentFileService:
    ...
    def _check_chat_id(self):
        ...
        if chat_obj.type != ChatType.DataAnalytics:
            raise HTTPException(
                status_code=400, detail=f"Chat object under {self.chat_id} id does not have FileSearch as its chat type"
            )

    def download_file(self, stored_file_id: str):
        chat_message_with_store_id = self.session.query(Message).filter(
            Message.chat_id == self.chat_id, Message.stored_file_id == stored_file_id
        ).first()
