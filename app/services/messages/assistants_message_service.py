...
class AssistantMessageService:
    ...
    def create_user_message(
        ...
        for document_metadata in relevant_docs:
            assistant_file_obj = self.session.query(AssistantFile).filter(
                AssistantFile.id == document_metadata["fileId"]
            ).first()
            if assistant_file_obj:
                asst_message_document_obj = MessageAssistantDocument(
                    document_id=document_metadata["id"],
                    message=new_agent_message,
                    assistant_file=assistant_file_obj
                )
    ...
    def get_messages(self):
        ...
        return ChatMapper.map_to_assistant_chat_history_response(
            chat=self.chat_obj, messages=message_objs
        )