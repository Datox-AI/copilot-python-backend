...
class AssistantService:
    ...
    def get_assistants(self):
        assistants = self.session.query(Assistant).filter(Assistant.created_by==self.user.user_id).all()
        return [
            AssistantMapper.map_to_assistant_response(
                assistant=assistant,
                assistant_knowledge_files=assistant.knowledge_files  
            ) 
            for assistant in assistants
        ]
        
    def get_assistant_chats(self, assistant_id: str):
        assistant = self.session.query(Assistant).filter(Assistant.assistant_id==assistant_id).first()
        if assistant:
    ...
    async def update_assistant_files(self, assistant_id: str, files_to_delete: List[str], new_files: List[UploadFile]):
        ...
        if assistant_obj is not None:
            ...
            self.session.commit()
            return AssistantMapper.map_to_assistant_response(
                assistant=assistant_obj, assistant_knowledge_files=assistant_obj.knowledge_files
            )        
