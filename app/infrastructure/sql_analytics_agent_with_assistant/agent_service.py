...
class DataAnalyticAssistant:
    def __init__(self, snowflake_engine: Engine, thread_id: Union[str, None]):
        ...
        container_name = os.getenv("AZURE_STORAGE_DA_ASSISTANT_AGENT_CONTAINER")
        azure_blob_storage_manager = AzureBlobStorageManager(container_name)
        token_counter = TokenCounter()

        self.db.initiate_blob_storage_manager_and_token_counter(
            blob_manager=azure_blob_storage_manager, token_counter=token_counter
        )
    ...
    def execute_agent(self, input: str, message_id: UUID):
            ...
            try:
                ...
                for action in response:
                    if action.tool == "sql_db_query":
                        ...
                        tool_response = tool_map[action.tool].invoke(
                            input={"query": action.tool_input, "message_id": message_id.hex}
                        )
        ...
        followup_questions = self.generate_followup_questions(question=input, answer=response.return_values["output"])
